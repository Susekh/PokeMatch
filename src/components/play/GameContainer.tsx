...
export default function GameContainer() {
  ...
  useEffect(() => {
    ...
    gsap.fromTo(".game-title", { opacity: 0, y: -20 }, { opacity: 1, y: 0, duration: 0.5 })

    gsap.fromTo(".game-subtitle", { opacity: 0 }, { opacity: 1, duration: 0.5, delay: 0.3 })
  }, [])
        ...
        <div className="game-board">
          <div className="flex flex-col md:flex-row gap-6 items-start">
            <div className="w-full md:w-3/4">
              <div className="flex justify-between items-center mb-4">
                <Button
                  variant="ghost"
                  onClick={resetGame}
                  className="text-indigo-200 hover:text-indigo-100 hover:bg-indigo-900/30"
                >
            ...
            <div className="w-full md:w-1/4">
              <ScoreTracker scores={scores} currentPlayer={currentPlayer} gameMode={gameMode} level={level} />
            </div>
          </div>
        </div>
      )}

      {gameState === "levelUp" && showLevelUp && <LevelUpAnimation level={level + 1} />}
          ...
          <div className="mb-8">
            ...
            <p className="text-xl text-indigo-200">
              {gameMode === "multiplayer"
                ? `${scores.player1 > scores.player2 ? "Player 1" : "Player 2"} wins!`
